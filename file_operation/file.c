...
#define SMBIOS_STRUCTURE "/sys/firmware/dmi/tables/DMI"
...
int main(int argc, char *argv[])
{
        int fd;
        int ret;
        fd = open(SMBIOS_STRUCTURE, O_RDONLY);
        if (fd < 0) {
                printf("open file %s fail: %s!", SMBIOS_STRUCTURE, strerror(errno));
                return -1;
        }

        ret = write_file(fd, "./smbios_structre");
        if (ret < 0) {
